  ...
  dbQuery = {
    ...
    async exec(sql) {
      if (/^\s*(begin|commit|rollback)/i.test(sql)) {
        return; // Ignore transaction controls on PostgreSQL Pool to prevent connection boundary issues
      }
...
// Clean and parse numeric values robustly (removes currency prefixes, commas, and spaces)
function parseNumeric(val, defaultVal = 0.0) {
...
app.post('/api/stores/import', authenticateToken, requireRole(['DECPHMD1']), upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    for (let row of data) {
      ...
      const qtyAlloc = parseNumeric(getFuzzyValue(row, ['Quantity Allocated', 'quantity_allocated', 'Allocated Quantity', 'Allocated', 'Qty Allocated', 'Allocated Qty'], 0.0));
      const qtyOnHand = parseNumeric(getFuzzyValue(row, ['Quantity On Hand', 'quantity_on_hand', 'OnHand Quantity', 'OnHand', 'Qty On Hand', 'OnHand Qty', 'On Hand'], 0.0));
      const price = parseNumeric(getFuzzyValue(row, ['Unit Price', 'unit_price', 'price', 'Rate', 'Unit Rate', 'Cost'], 0.0));
      const val = parseNumeric(getFuzzyValue(row, ['Value', 'value', 'Total Value', 'Amount', 'Total Amount'], (qtyOnHand * price)));
